(() => {
  ...
  function setupSightSlider() {
    ...
    track.addEventListener("transitionend", (e) => {
      if (e.propertyName === "transform") normalizeSightSlider();
    });
    updateSightSlider();
  }
  ...
  function update() {
    ...
    root.style.setProperty("--bazaar-brightness", String(1 - frame2.active * 0.255 - frame3.active * 0.06));
    ...
    const stillScrolling = Math.abs(smoothScroll - targetScroll) > 0.08;
    const stillMouse = Math.abs(mouseX - targetMouseX) > 0.001 || Math.abs(mouseY - targetMouseY) > 0.001;
    if (stillScrolling || stillMouse) requestTick();
  }
